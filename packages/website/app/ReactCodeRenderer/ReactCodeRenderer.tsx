...
import { DynamicComponentRendererProps, ModuleCache, ExportsObject } from './interface';
    ...
    const processFile = (filename: string): any => {
      ...
      ComponentModule(
        (importPath: string) => {
          ...
          const normalizedPath = Object.keys(files).find((file) => possiblePaths.includes(file));
          ...
          if (normalizedPath) {
            try {
              ...
              if (
                result === undefined ||
                (typeof result === 'object' && Object.keys(result).length === 0 && !result.default)
              ) {
                throw new Error(
          ...
          try {
            ...
            if (error instanceof Error) {
              ...
              throw new Error(`Error loading external module "${importPath}": ${error.message}`);
            }
            throw error;
          }
        },
    ...
    const parseComponents = async () => {
      try {
        ...
        if (missingComponentMatch) {
          ...
          const importSource = importSourceMatch ? importSourceMatch[1] : 'a module';
